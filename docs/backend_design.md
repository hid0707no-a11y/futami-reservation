...
> ⚠️ **2026-09-02 追記：本書に出てくる `data.json` 方式は廃止済み。復活させない。**
> 本書は 2026-04 の移行時点の設計記録であり、以下の記述は歴史的経緯として残している。
> `data.json` にはカレンダー由来の実顧客データ（氏名・電話・メール）が入っており、
> public リポジトリ＋GitHub Pages 直下配信のため 148日間公開されていた（2026-09-02 に除去）。
> **在庫の正本は `/availability` API のみ**。§8 の「切り戻し保険」は無効。詳細＝
> `nissho/00_projects/futami_reservation/docs/2026-09-02_インシデント_data.json個人情報露出.md`
