...
// 半面練習プランが使うのは「壁打ち練習用の半面コート」1つだけ（index.html の
// tennis_half.rooms と一致必須）。A〜Eの5面とは別の独立施設なので court_1 ではない。
// 根拠：公式サイト施設案内「通常のテニスコートが5面、壁打ち練習用の半面コート1つ」／
//       公式料金表【R8】の「半面（練習用）」別行／運営（上村さん）2026-07-21 回答
//       「半面コートは一面しかない」「一面しか無いため、1組まで で問題なし」。
const HALF_COURT_ROOMS = ['court_wall'] as const;
...
export const RESERVATION_PLAN_RULES: Readonly<Record<string, PlanRule>> = {
  ...
  // 2026-07-21 是正：貸出単位は court_wall（壁打ち練習用の半面コート）1つのみ。
  // これは A〜E の5面とは別に存在する独立施設なので、tennis_slots のキーは
  // court_wall|date|HHMM となり全面(court_1〜court_5|date|HHMM)とキー空間が自然に分離する。
  //   → 半面1件がコートAを塞ぐ不当な在庫消費が無くなる（全面と同一時間帯に併存可）
  //   → 実体である壁打ちコートに排他制御が掛かる（半面2件目は同一キー衝突で409）
  // 「1組まで」は運営（上村さん）2026-07-21 回答②に基づく（maxRooms=1・在庫1つ）。
